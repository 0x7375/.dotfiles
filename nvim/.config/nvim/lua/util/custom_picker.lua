...
local function calculate_dimensions(picker)
    ...
    return window_width, window_height, x_centered, y_centered, preview_height, preview_height_with_border,
        results_height
end
...
function M.create_layout(picker)
    ...
    local layout = Layout {
        ...
        mount = function(self)
            if picker.previewer then
                self.preview = create_window(false, window_width, preview_height, y_centered, x_centered,
                    PREVIEW_BORDER)
            end
            self.prompt = create_window(true, window_width, prompt_height,
                y_centered + preview_height_with_border,
                x_centered, PROMPT_BORDER)
            self.results = create_window(false, window_width, results_height,
                y_centered + preview_height_with_border + prompt_height_with_border,
                x_centered, RESULTS_BORDER)
        end,
        ...
        update = function(self)
            ...
            if picker.previewer then
                if not self.preview then
                    ...
                    update_window(self.preview, window_width, preview_height, y_centered, x_centered,
                        PREVIEW_BORDER)
                end
            else
            ...
            update_window(self.prompt, window_width, prompt_height, y_centered + preview_height_with_border,
                x_centered, PROMPT_BORDER)

            update_window(self.results, window_width, results_height,
                y_centered + preview_height_with_border + prompt_height_with_border,
                x_centered, RESULTS_BORDER)
        end,
    }
